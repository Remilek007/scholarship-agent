...
function canonicalUrl(value: string): string { try { const url = new URL(value); url.hash = ""; url.hostname = url.hostname.toLowerCase(); for (const key of [...url.searchParams.keys()]) if (TRACKING.test(key)) url.searchParams.delete(key); if (url.pathname.length > 1) url.pathname = url.pathname.replace(/\/+$/, ""); return url.toString(); } catch { return value.trim(); } }
...
export async function crawlDiscoveryPages(seedUrls: string[], query: string, options: CrawlOptions): Promise<CrawlResult> {
  ...
  const handler = async ({ request, $, enqueueLinks }: any): Promise<void> => {
    ...
    try {
      const title: string = $("title").first().text().trim() || $("h1").first().text().trim();
      const main: string = $("main").text(" ").trim() || $("article").text(" ").trim() || $("body").text(" ").trim();
  ...
  if (options.playwrightEnabled) {
    ...
    if (browserUrls.length) {
      const browser = new PlaywrightCrawler({ maxRequestsPerCrawl: browserUrls.length, maxConcurrency: Math.max(1, Math.min(options.concurrency, 4)), maxCrawlDepth: options.maxDepth, requestHandlerTimeoutSecs: Math.ceil(options.requestTimeoutMs / 1000), maxRequestRetries: 1,
        ...
        failedRequestHandler: async ({ request, error }: any): Promise<void> => { failures.push({ url: request.url, error: error instanceof Error ? error.message : String(error), stage: "playwright" }); }
      });
