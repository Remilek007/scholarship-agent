...
const MAX_ENRICH=40, CONCURRENCY=5;
export async function enrichDiscoveryRecords(profile:ApplicantProfile,records:DiscoveryRecord[],limit=MAX_ENRICH):Promise<EnrichedDiscoveryRecord[]>{
 const selected=records.slice(0,Math.max(1,Math.min(limit,MAX_ENRICH)));const results:EnrichedDiscoveryRecord[]=new Array(selected.length);let cursor=0;
 async function worker(){while(true){const index=cursor++;if(index>=selected.length)return;const record=selected[index];try{
  const extraction=await deepExtractPage(record.url);const enrichedRecord:DiscoveryRecord={...record,url:extraction.finalUrl||record.url,title:extraction.title||record.title,snippet:[record.snippet,extraction.text].filter(Boolean).join(" ").slice(0,30000)};
