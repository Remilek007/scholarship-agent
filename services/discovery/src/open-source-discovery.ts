...
function withTimeout<T>(promise: Promise<T>, timeoutMs: number): Promise<T> {
  return Promise.race([
    ...
    new Promise<never>((_, reject) =>
      setTimeout(() => reject(new Error(`Discovery request timed out after ${timeoutMs}ms`)), timeoutMs),
    ),
  ]);
}
...
export async function searchWithSearXNG(
  query: string,
  options: OpenSourceDiscoveryOptions = {},
  ...
  endpoint.searchParams.set("engines", (options.engines ?? DEFAULT_ENGINES).join(","));
...
export async function discoverBroadly(
  queries: string[],
  options: OpenSourceDiscoveryOptions = {},
): Promise<DiscoverySearchHit[]> {
...
export function toNormalizedDiscoveryRecords(hits: DiscoverySearchHit[]) {
  return normalizeDiscoveryRecords(
    hits.map((hit) => ({
      title: hit.title,
      url: hit.url,
      sourceUrl: hit.url,
