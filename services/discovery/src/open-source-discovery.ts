...
export type OpenSourceDiscoveryOptions = { searxngUrl?: string; engines?: string[]; maxResults?: number; timeoutMs?: number; };
export type DiscoverySearchHit = { title: string; url: string; snippet?: string; engine?: string; };
const DEFAULT_ENGINES = ["google", "bing", "brave", "duckduckgo"];

function withTimeout<T>(promise: Promise<T>, timeoutMs: number): Promise<T> {
  return Promise.race([promise, new Promise<never>((_, reject) => setTimeout(() => reject(new Error(`Discovery request timed out after ${timeoutMs}ms`)), timeoutMs))]);
}
...
export async function searchWithSearXNG(query: string, options: OpenSourceDiscoveryOptions = {}): Promise<DiscoverySearchHit[]> {
  ...
  return (payload.results ?? []).filter(item => Boolean(item.title && item.url)).slice(0, options.maxResults ?? 100).map(item => ({ title: item.title!, url: cleanUrl(item.url!), snippet: item.content, engine: item.engine }));
}
...
export async function discoverBroadly(queries: string[], options: OpenSourceDiscoveryOptions = {}): Promise<DiscoverySearchHit[]> {
  ...
  for (const hit of hits.flat()) { const key = cleanUrl(hit.url); if (!unique.has(key)) unique.set(key, hit); }
  return [...unique.values()];
}

export function toNormalizedDiscoveryRecords(hits: DiscoverySearchHit[]) {
  return normalizeDiscoveryRecords(hits.map(hit => ({ title: hit.title, url: hit.url, sourceUrl: hit.url, snippet: hit.snippet ?? "", discoveryMethod: "searxng", source: "searxng", sourceEngine: hit.engine })));
}
