...
const discoveryIntents = ["scholarship", "funded master's", "funded MSc", "graduate research position", "research assistantship", "studentship", "fellowship", "funded MSc position", "funded master's research position", "MSc student funded project", "MSc studentship forestry", "graduate research assistant forestry", "supervisor funded MSc forestry", "funded graduate student forest ecology", "funded MSc wildlife conservation", "research assistantship forest management", "funded thesis forestry"];
...
export function buildDiscoveryQueries(profile: ApplicantProfile): string[] {
  ...
  for (const field of selected) {
    for (const funding of fundingTerms) { queries.add(`"${funding}" "${field}" ${degree} "${nationality}"`); queries.add(`"${field}" "${funding}" international students`); }
    for (const intent of discoveryIntents) { queries.add(`"${field}" "${intent}" ${nationality}`); queries.add(`"${field}" "${intent}" international students`); }
    queries.add(`"${field}" funded research project master's supervisor`); queries.add(`"${field}" MSc scholarship university department funding`); queries.add(`"${field}" funded graduate position professor lab`);
    for (const domain of authoritativeDomains) { queries.add(`site:${domain} "${field}" "funded" ${degree}`); queries.add(`site:${domain} "${field}" scholarship "international students"`); }
  }
  return [...queries];
}
