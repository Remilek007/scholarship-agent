...
function textIncludes(candidate: ScholarshipCandidate, term: string): boolean { const haystack = [candidate.title, ...(candidate.fields ?? []), candidate.eligibility?.text ?? ""].join(" ").toLowerCase(); return haystack.includes(term.toLowerCase()); }
