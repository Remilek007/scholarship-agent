...
export interface DocumentFact { field: "highestQualification" | "degreeField" | "academicScore" | "academicScale" | "targetFields" | "workExperience"; value: string; confidence: number; evidence: string; }
...
const FIELD_PATTERNS: Array<[RegExp, string]> = [
  [/\bforestry\b/i, "Forestry"], [/\bforest science\b/i, "Forest Science"], [/\bforest management\b/i, "Forest Management"], [/\bsilviculture\b/i, "Silviculture"], [/\bforest ecology\b/i, "Forest Ecology"], [/\btropical forestry\b/i, "Tropical Forestry"], [/\bwildlife\b/i, "Wildlife"], [/\bzoology\b/i, "Zoology"], [/\bconservation\b/i, "Conservation"], [/\bbiodiversity\b/i, "Biodiversity"], [/\bnatural resources?\b/i, "Natural Resources"], [/\becosystem management\b/i, "Ecosystem Management"], [/\bagroforestry\b/i, "Agroforestry"], [/\bcommunity forestry\b/i, "Community Forestry"], [/\bforest carbon\b/i, "Forest Carbon"], [/\bREDD\+?/i, "Forest Carbon / REDD+"], [/\bremote sensing\b/i, "Remote Sensing"], [/\bGIS\b|geographic information systems?/i, "Geospatial/GIS"], [/\bQGIS\b/i, "QGIS"], [/\bArcGIS\b/i, "ArcGIS"], [/\bclimate change\b/i, "Climate"], [/\bclimate adaptation\b/i, "Climate Adaptation"], [/\brestoration ecology\b/i, "Restoration Ecology"], [/\benvironmental management\b/i, "Environmental Management"], [/\benvironmental policy\b/i, "Environmental Policy"]
];
const SKILL_PATTERNS: Array<[RegExp, string]> = [[/\bQGIS\b/i,"QGIS"],[/\bArcGIS\b/i,"ArcGIS"],[/\bR programming\b|\bRStudio\b/i,"R / RStudio"],[/\bPython\b/i,"Python"],[\bSPSS\b/i,"SPSS"],[\bremote sensing\b/i,"Remote Sensing"],[\bGPS\b/i,"GPS / Field Data"],[\bdata analysis\b/i,"Data Analysis"]];
...
export function analyzeApplicantDocument(text: string, hintedType?: DocumentAnalysis["documentType"]): DocumentAnalysis {
  ...
  const add = (field: DocumentFact["field"], value: string | undefined, confidence: number, evidence: string) => { if (value?.trim()) facts.push({ field, value: value.trim(), confidence, evidence: evidence.trim().slice(0, 360) }); };
  ...
  if (skills.length) facts.push({ field: "workExperience", value: `Technical skills: ${skills.join(", ")}`, confidence: 0.82, evidence: skills.join(", ") });
  ...
  return { documentType, facts, suggestedProfilePatch, warnings: ["Review every extracted fact before saving it to your profile.", "Only explicit document evidence is used; nationality, achievements, referees, research claims and other unsupported facts are never invented."] };
}
function extractSection(text: string, starts: string[], ends: string[]): string | undefined { const start = new RegExp(`(?:${starts.map(escape).join("|")})[:\\s]+`, "i").exec(text); if (!start) return undefined; const rest = text.slice(start.index + start[0].length); const end = new RegExp(`\\s(?:${ends.map(escape).join("|")})[:\\s]`, "i").exec(rest); return rest.slice(0, end ? end.index : 600).trim(); }
