...
export function createScholarshipRepository(databaseUrl?: string) {
  ...
  return {
    async upsertScholarship(input: ScholarshipPersistenceInput) {
      const [scholarship] = await db.insert(scholarships).values({ canonicalKey: input.canonicalKey, title: input.title, provider: input.provider, university: input.university, country: input.country, degreeLevel: input.degreeLevel, opportunityType: input.opportunityType ?? "scholarship", fields: input.fields, sourceUrl: input.sourceUrl, applicationUrl: input.applicationUrl, fundingClass: input.fundingClass, deadline: input.deadline ? new Date(input.deadline) : undefined, updatedAt: new Date() }).onConflictDoUpdate({ target: scholarships.canonicalKey, set: { title: input.title, provider: input.provider, university: input.university, country: input.country, degreeLevel: input.degreeLevel, opportunityType: input.opportunityType ?? "scholarship", fields: input.fields, sourceUrl: input.sourceUrl, applicationUrl: input.applicationUrl, fundingClass: input.fundingClass, deadline: input.deadline ? new Date(input.deadline) : undefined, updatedAt: new Date() } }).returning({ id: scholarships.id });
    ...
    async recordVerification(scholarshipId: string, verification: SourceVerificationInput) {
      ...
      await db.insert(scholarshipSnapshots).values({ scholarshipId, sourceUrl: verification.finalUrl ?? verification.sourceUrl, title: verification.title ?? "Source verification", snippet: verification.warnings.join(" ").slice(0, 8000) || undefined, evidence: { verification: { status: verification.status, trustLevel: verification.trustLevel, officialSource: verification.officialSource, sourceUrl: verification.sourceUrl, finalUrl: verification.finalUrl, evidence: verification.evidence, warnings: verification.warnings, checkedAt: verification.checkedAt } } });
      return this.getScholarship(scholarshipId);
    },
    async getScholarship(id: string) {
      const rows = await db.select().from(scholarships).where(eq(scholarships.id, id)).limit(1); if (!rows[0]) return undefined;
      const [sources, funding, snapshots] = await Promise.all([db.select().from(scholarshipSources).where(eq(scholarshipSources.scholarshipId, id)), db.select().from(scholarshipFunding).where(eq(scholarshipFunding.scholarshipId, id)).limit(1), db.select().from(scholarshipSnapshots).where(eq(scholarshipSnapshots.scholarshipId, id)).orderBy(desc(scholarshipSnapshots.capturedAt)).limit(10)]);
    ...
    async getLatestEligibilityEvidence(ids: string[]) {
      ...
      const rows = await db.select({ scholarshipId: scholarshipSnapshots.scholarshipId, evidence: scholarshipSnapshots.evidence }).from(scholarshipSnapshots).where(inArray(scholarshipSnapshots.scholarshipId, ids)).orderBy(desc(scholarshipSnapshots.capturedAt));
    ...
    async getApplication(id: string) {
      ...
      const [requirements, answers, events] = await Promise.all([db.select().from(applicationRequirements).where(eq(applicationRequirements.applicationId, id)).orderBy(applicationRequirements.createdAt), db.select().from(applicationAnswers).where(eq(applicationAnswers.applicationId, id)).orderBy(applicationAnswers.field), db.select().from(applicationEvents).where(eq(applicationEvents.applicationId, id)).orderBy(desc(applicationEvents.createdAt)).limit(100)]);
