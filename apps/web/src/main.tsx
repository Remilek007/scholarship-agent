...
type Match = {
  ...
  reasons: string[]; sourceUrl: string; applicationUrl?: string; opportunityType?: string;
};
...
function App() {
  ...
  async function load() {
    ...
    try {
      ...
      const response = await fetch(`${API}/api/matches?${params}`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(PROFILE) });
  ...
  return <main>
    <header className="hero"><div><p className="eyebrow">SCHOLARSHIP AGENT</p><h1>Your funded Master's shortlist.</h1><p className="sub">Forestry-first intelligence across scholarships, universities, research positions and funding sources.</p></div><button onClick={() => void load()} disabled={loading}>{loading ? "Matching…" : "Find best matches"}</button></header>
    <section className="stats"><div><strong>{items.length}</strong><span>top matches</span></div><div><strong>{ready}</strong><span>ready for review</span></div><div><strong>{review}</strong><span>need verification</span></div></section>
    <section className="toolbar"><div><label>Funding</label><select value={funding} onChange={e => setFunding(e.target.value)}><option value="all">All qualifying</option><option value="fully_funded">Fully funded</option><option value="substantially_funded">Substantial funding</option></select></div><label className="check"><input type="checkbox" checked={includeReview} onChange={e => setIncludeReview(e.target.checked)} /> Include uncertain eligibility</label><p>Ranked for Nigeria · Master's · Forestry/Wildlife + related fields</p></section>
    ...
    <section className="grid">{visible.map(item => <article className="card" key={item.id}><div className="cardtop"><span className="score">{item.score}% match</span><span className="pill">{labelFunding(item.fundingClass)}</span><span>Trust {item.trustLevel}/5</span></div><h2>{item.title}</h2><p className="muted">{[item.provider, item.university, item.country].filter(Boolean).join(" · ") || "Provider not yet verified"}</p><div className="status">{labelEligibility(item.eligibility)}</div><div className="tags">{item.fields.slice(0, 6).map(field => <span key={field}>{field}</span>)}</div><ul className="reasons">{item.reasons.slice(0, 3).map(reason => <li key={reason}>{reason}</li>)}</ul><footer><span>{item.deadline ? `Deadline: ${new Date(item.deadline).toLocaleDateString()}` : "Deadline not yet verified"}</span><span className="actions"><a href={item.sourceUrl} target="_blank" rel="noreferrer">Source</a>{item.applicationUrl && <a href={item.applicationUrl} target="_blank" rel="noreferrer">Apply</a>}<button onClick={() => void openApplication(item.id)} disabled={appLoading}>Prepare application</button></span></footer></article>)}</section>
...
function ApplicationPanel({ application, onClose, onUpdate }: { application: Application; onClose: () => void; onUpdate: (application: Application) => void }) {
  ...
  async function save() { setSaving(true); try { const response = await fetch(`${API}/api/applications/${application.id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ status, notes }) }); if (!response.ok) throw new Error("Unable to save application"); const data = await response.json() as { application: Application }; onUpdate(data.application); } finally { setSaving(false); } }
  return <div className="overlay"><section className="workspace"><header><div><p className="eyebrow">APPLICATION WORKSPACE</p><h2>Prepare this application</h2><p className="muted">Draft and review here. Final submission remains under your control.</p></div><button onClick={onClose}>Close</button></header><div className="workspace-grid"><div><h3>Application status</h3><select value={status} onChange={e => setStatus(e.target.value)}><option value="discovered">Discovered</option><option value="preparing">Preparing</option><option value="ready_for_review">Ready for review</option><option value="submitted">Submitted</option><option value="withdrawn">Withdrawn</option></select><h3>Requirements</h3><ul className="requirements">{application.requirements.length ? application.requirements.map(req => <li key={req.id}><strong>{req.name}</strong><span>{req.required ? "Required" : "Optional"} · {req.status}</span></li>) : <li>No requirements extracted yet.</li>}</ul></div><div><h3>Answers</h3>{application.answers.length ? application.answers.map(answer => <article className="answer" key={answer.id}><strong>{answer.field}</strong><p>{answer.answer}</p><small>{answer.reviewed ? "Reviewed" : "Needs review"} · AI policy: {answer.aiPolicy}</small></article>) : <div className="empty">No answers prepared yet. The next step is extracting application questions from the official source.</div>}<h3>Notes</h3><textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Your application notes…" /><button onClick={() => void save()} disabled={saving}>{saving ? "Saving…" : "Save application"}</button></div></div></section></div>;
}
